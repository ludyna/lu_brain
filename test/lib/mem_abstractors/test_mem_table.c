#include "unity.h"
#include "lib/_module.h"

void setUp(void)
{
    
}

void tearDown(void)
{
}

struct test1 {
    lu_size table_size_in_records;
};

typedef struct test1* Test1;

void test_mem_table1(void)
{
    Lu_Mem mem        = (Lu_Mem) lu_mem_stack__create	(lu_g_mem, 512);
    Lu_Mem_Table mt   = lu_mem_table__create(
        /*Lu_Mem*/                  mem, 
        /*record_size_in_bytes*/    sizeof(struct test1), 
        /*table_size_in_records*/   3, 
        /*percent*/                 0, 
        /*flags*/                   LU_MEM_TABLE__FREEABLE
    );

    TEST_ASSERT( ((Lu_Mem_Stack_Table)mt)->mem_stack == (Lu_Mem_Stack) mem );

    TEST_ASSERT(mt->table_size_in_records == 3);
    TEST_ASSERT(mt->free_count == 0);
    TEST_ASSERT(mt->records_count == 0);

    Test1 l1 = (Test1) lu_mem_record__alloc(mt);

    lu__debug("%ld %ld %p", (lu_size)l1, (lu_size)(mt->records_start + (mt->table_size_in_records - 1) * mt->record_size_in_bytes), mt->free_start);
    TEST_ASSERT( ((lu_size)l1) == (lu_size)(mt->records_start));
    TEST_ASSERT(mt->table_size_in_records == 3);
    TEST_ASSERT(mt->free_count == 0);
    TEST_ASSERT(mt->records_count == 1);
    TEST_ASSERT(l1 != NULL);

    Test1 l2 = (Test1) lu_mem_record__alloc(mt);

    TEST_ASSERT( ((lu_size)l2) == (lu_size)(mt->records_start + mt->record_size_in_bytes) );
    TEST_ASSERT(mt->table_size_in_records == 3);
    TEST_ASSERT(mt->free_count == 0);
    TEST_ASSERT(mt->records_count == 2);
    TEST_ASSERT(l2 != NULL);

    Test1 l3 = (Test1) lu_mem_record__alloc(mt);

    TEST_ASSERT( ((lu_size)l3) == (lu_size)(mt->records_end - mt->record_size_in_bytes) );
    TEST_ASSERT(mt->table_size_in_records == 3);
    TEST_ASSERT(mt->free_count == 0);
    TEST_ASSERT(mt->records_count == 3);
    TEST_ASSERT(l3 != NULL);

    lu_mem_record__free(mt, (lu_p_byte) l3);

    TEST_ASSERT(mt->table_size_in_records == 3);
    TEST_ASSERT(mt->free_count == 1);
    TEST_ASSERT(mt->records_count == 2);
    TEST_ASSERT( ((lu_size)l3) == (lu_size)(mt->free_start[0]) );

    lu_mem_record__free(mt, (lu_p_byte) l2);

    TEST_ASSERT(mt->table_size_in_records == 3);
    TEST_ASSERT(mt->free_count == 2);
    TEST_ASSERT(mt->records_count == 1);
    TEST_ASSERT( ((lu_size)l2) == (lu_size)(mt->free_start[1]) );

    l2 = (Test1) lu_mem_record__alloc(mt);
    l1 = (Test1) lu_mem_record__alloc(mt);

    TEST_ASSERT(mt->table_size_in_records == 3);
    TEST_ASSERT(mt->free_count == 0);
    TEST_ASSERT(mt->records_count == 3);

    lu__user_assert_off();
    Test1 l4 = (Test1) lu_mem_record__alloc(mt);
    lu__user_assert_on();
    
    TEST_ASSERT(l4 == NULL);

    lu_mem_record__free(mt, (lu_p_byte) l4);
    lu_mem_record__free(mt, (lu_p_byte) l3);
    lu_mem_record__free(mt, (lu_p_byte) l2);
    lu_mem_record__free(mt, (lu_p_byte) l1);

    // TEST_ASSERT(mt->table_size_in_records == 12);
    // TEST_ASSERT(mt->free_count == 8);
    // TEST_ASSERT(mt->records_count == 0);

    lu_mem__destroy(mem, lu_g_mem);
}
